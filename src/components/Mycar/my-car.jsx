...
const MyCar = () => {
  ...
  // Reset selections if any data is in localStorage
  useEffect(() => {
    const resetSelections = () => {
      // Check if any of the selections are stored in localStorage
      ...
      const savedEcu = localStorage.getItem("selectedEcu");

      // If any value is found, reset the corresponding state to null
      if (
        savedCategory ||
        savedBrand ||
        savedModel ||
    ...
    const fetchBrands = async () => {
      if (selectedCategory !== null) {
        try {
          setIsLoading(true);
          const response = await axios.get(
            "http://127.0.0.1:8000/api/vehicles/names",
            {
    ...
    const fetchModels = async () => {
      if (selectedCategory && selectedBrand) {
        try {
          setIsLoading(true);
          const response = await axios.get(
            "http://127.0.0.1:8000/api/vehicles/models",
            {
              params: {
    ...
    const fetchGenerations = async () => {
      if (selectedModel) {
        try {
          setIsLoading(true);
          const response = await axios.get(
            "http://127.0.0.1:8000/api/vehicles/generations",
            {
    ...
    const fetchEngines = async () => {
      if (selectedGeneration) {
        try {
          setIsLoading(true);
          const response = await axios.get(
            "http://127.0.0.1:8000/api/vehicles/engines",
            {
              params: {
    ...
    const fetchEcUs = async () => {
      if (selectedEngine) {
        try {
          setIsLoading(true);
          const response = await axios.get(
            "http://127.0.0.1:8000/api/vehicles/ecus",
            {
              params: {
  ...
  // Save selections in localStorage on button click
  const saveSelectionsToLocalStorage = () => {
    ...
    localStorage.setItem("selectedEcu", selectedEcu);
  };
          ...
          {categories.map((category) => (
            ...
            >
              <div className="d-flex flex-column align-items-center">
                <img
                  src={`/img/${category.category_name
                    .toLowerCase()
                    .replace(" ", "-")}.png`}
          ...
          <div className="col-md-12 text-center">
            <Link href="/car/cardetails">
              <button
                ...
                onClick={saveSelectionsToLocalStorage}
              >
